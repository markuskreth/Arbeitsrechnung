...
public class KlientPersister extends AbstractPersister {
	...
	public Einheit getEinheitById(int einheit) {
		String sqltext = "SELECT einheiten_id,klienten_id,angebote_id,Datum,Beginn,Ende,Preisänderung,zusatz1,zusatz2,Rechnung_Datum,Bezahlt_Datum "
		...
		try {
			...
			Einheit e = new Einheit.Builder().angebotId(daten.getInt("angebote_id"))
					...
					.klientenId(daten.getInt("klienten_id")).datum(daten.getDate("Datum"))
					.preisAenderung(daten.getDouble("Preisänderung")).rechnungDatum(daten.getDate("Rechnung_Datum"))
					.zusatz1(daten.getString("zusatz1")).zusatz2(daten.getString("zusatz2"))
					.bezahltDatum(daten.getDate("Bezahlt_Datum")).build();
