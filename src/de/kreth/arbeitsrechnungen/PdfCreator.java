	...
	public int makePdf(Options optionen) {
      ...
      latexcode = latexcode.replaceAll("%%adresse%%\n", trimAdressAndTexify(rechnung.getAdresse()));
		...
		String repl = "\\\\setkomavar{date}{"
				+ df.format(rechnung.getDatum().getTime()) + "}";
		...
		if (std_ersetzung) {
			...
			if (rechnung.isStunden())
				latexcode = latexcode.replaceAll(
						...
						" für " + zf.format(this.summe_stunden) + " Std. insgesamt "
								+ zf.format(rechnung.getBetrag()) + TEXEURO);
			else
				latexcode = latexcode.replaceAll("%%summe%%",
						"insgesamt " + zf.format(rechnung.getBetrag()) + TEXEURO);
			latexcode = latexcode.replaceAll("%%zahlfrist%%",
					df.format(rechnung.getZahldatum().getTime()));
		} else {
			...
			latexcode = latexcode.replaceAll(ersetzung, zf.format(rechnung.getBetrag())
					+ " " + TEXEURO);
			ersetzung = "\"Monat\"";
		...
		if (std_ersetzung) {
		   ...
		   makeLatexTableHead(tabelle, spalten);
			...
			for (int i = 0; i < rechnung.getEinheiten().size(); i++) {
				...
				if (rechnung.isStunden()){
				   dauer = BigDecimal.valueOf(rechnung.getEinheiten().elementAt(i).getDauerInMinutes()).divide(BigDecimal.valueOf(60)).setScale(2, RoundingMode.HALF_UP).stripTrailingZeros();
				...
				tabelle.append(zf.format(rechnung.getEinheiten().elementAt(i).getPreis()) + " "
						+ TEXEURO + TEXUMBRUCH + TEXLINE + "\n");
				
				summe_stunden = summe_stunden.add(BigDecimal.valueOf(dauer.doubleValue())); // Minuten in Stunden
			}
         ...
         double summe = rechnung.getBetrag().doubleValue();
			...
			for (int i = 0; i < rechnung.getEinheiten().size(); i++) {
				...
				zeile = zeile.replaceAll("\"Datum\"",
						df.format(rechnung.getEinheiten().elementAt(i).getDatum()));
				
				if (rechnung.getZusatz1_name().matches("Teilnehmerzahl"))
					zeile = zeile.replaceAll("\"Teilnehmerzahl\"", rechnung.getEinheiten()
							.elementAt(i).getZusatz1());
				
				zeile = zeile.replaceAll("\"Dauer\"", Double.valueOf(rechnung.getEinheiten()
   ...
   private void checkEinheitenGleichheit(int i) {
	...
	private void markAsDoubleIfPreisAndZusaetzeMacht(int i, Arbeitsstunde element1, Arbeitsstunde element2) {
      ...
      if (preis.equals(element2.getPreis())) {
         ...
         if ((rechnung.getZusatz1_name() == null || element1
