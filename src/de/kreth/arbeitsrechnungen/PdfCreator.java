...
public class PdfCreator {
	...
	private boolean openTexFile() throws FileNotFoundException {
		...
		String texdatei = rechnung.getTexdatei();
		if(new File(texdatei).exists() == false) {
			logger.error("Texfile '" + texdatei + "' not found.");
		}
		latexdatei = new FileReader(texdatei);
		int c;
		try {
			...
			if (latexdatei != null) {
				try {
					...
					logger.error("Fehler beim Schließen der Tex-datei in " + texdatei, e);
				}
			}
	...
	/**
	 * Erstellt aus der Vorlage und den Daten eine Tex-Datei und führt pdflatex aus
	...
	public int makePdf(Options optionen) {
		...
		if (this.unterschrift) {
			closing += TEXUMBRUCH + "\\\\includegraphics[width=3.5cm]{" + optionen.getTexTemplatesDir() + File.separator
					+ "Sign.jpg}";
			String rep = PdfCreator.TEXUMBRUCH + "\\\\includegraphics[width=3.5cm]{" + optionen.getTexTemplatesDir()
		...
		if (std_ersetzung) {
			...
			if (rechnung.isStunden())
				latexcode = latexcode.replaceAll("%%summe%%", " für " + zf.format(this.summe_stunden)
						+ " Std. insgesamt " + zf.format(rechnung.getBetrag()) + TEXEURO);
			else
		...
		try {
			...
			if (!texFile.exists() || !texFile.isFile()) {
				throw new IllegalStateException(
						"TexFile " + texFile + " exists=" + texFile.exists() + "; isDir=" + texFile.isFile());
			}
	...
	private String createTable() {
		...
		if (std_ersetzung) {
			...
			for (int i = 0; i < rechnung.getEinheiten().size(); i++) {
				...
				tabelle.append(df.format(rechnung.getEinheiten().elementAt(i).getDatum()) + " & "
						+ rechnung.getEinheiten().elementAt(i).getInhalt() + " & ");

				BigDecimal dauer = BigDecimal.ZERO;
				if (rechnung.isStunden()) {
					dauer = BigDecimal.valueOf(rechnung.getEinheiten().elementAt(i).getDauerInMinutes())
							.divide(BigDecimal.valueOf(60)).setScale(2, RoundingMode.HALF_UP).stripTrailingZeros();
				...
				tabelle.append(zf.format(rechnung.getEinheiten().elementAt(i).getPreis()) + " " + TEXEURO + TEXUMBRUCH
						+ TEXLINE + "\n");
			...
			case 1: // Datum, Inhalt, Std, Preis
				tabelle.append(TEXLINE + "& & " + zf.format(summe_stunden) + " & " + zf.format(summe) + " " + TEXEURO
						+ TEXUMBRUCH + TEXLINE + "\n" + "\\\\end{tabular}");
				break;
			...
			case 6:
				...
				break;
			case 7:
				tabelle.append(TEXLINE + "& & " + zf.format(summe_stunden) + " & & & " + zf.format(summe) + " "
			...
			String zeilencode = this.latexcode.substring(this.latexcode.indexOf("%Zeileanfang"),
			...
			for (int i = 0; i < rechnung.getEinheiten().size(); i++) {
				...
				zeile = zeile.replaceAll("\"Dauer\"",
						Double.valueOf(rechnung.getEinheiten().elementAt(i).getDauerInMinutes() / 60.).toString());
				...
				this.summe_stunden = summe_stunden
						.add(BigDecimal.valueOf(rechnung.getEinheiten().elementAt(i).getDauerInMinutes() / 60));
				// Minuten in Stunden
	...
	private void checkEinheitenGleichheit(int i) {
		...
		if (i + 1 < rechnung.getEinheiten().size()) {
			...
			if (element1.getDatum().equals(element2.getDatum())
					&& element1.getAngeboteID() == element2.getAngeboteID()) {
	...
	private void markAsDoubleIfPreisAndZusaetzeMacht(int i, Arbeitsstunde element1, Arbeitsstunde element2) {
		...
		if (preis.equals(element2.getPreis())) {
			...
			if ((rechnung.getZusatz1_name() == null || element1.getZusatz1().equals(z1))
					&& (rechnung.getZusatz2_name() == null || element1.getZusatz2().equals(z2))) {
