...
/**
 * Diese klasse erzeugt pdf und kann das über externes Programm anzeigen und in Datenbank speichern.
 * @author markus
 *
 */
public class RechnungData {
	private Logger logger = Logger
			.getLogger("arbeitsrechnungen.RechnungData");
	...
	private Properties getEinstellungen() {
		...
		java.io.File optionfile = new java.io.File(
		...
		try {
			...
			System.err
					.println("ArbeitsstundenTabelle.java: Options-Datei konnte nicht geladen werden.");
			return null;
		}
	}

	/**
	 * Rechnung mit nötigen Daten füllen gibt True zurück, wenn die Tex-Datei
	...
	public boolean initRechnung(int klienten_id, String adresse,
			String tex_datei, Vector<ArbeitsstundeImpl> einheiten,
			GregorianCalendar heute, GregorianCalendar rechnungsdatum,
			GregorianCalendar zahlungsdatum, String rechnungsnr,
		...
		texfileSuccess = openTexFile(); // Texfile wird geöffnet und in Variable
										// latexcode gespeichert

		// Bekanntes wird ersetzt
		// latexcode =
		// latexcode.replaceAll("%\\setkomavar{invoice}{%%invoice%%}",
		// "\\setkomavar{invoice}{" +
		// this.rechnungsnr + "}");
	...
	public int makePdf() {
		...
		String repl = "\\\\setkomavar{date}{"
				+ df.format(rechnungsdatum.getTime()) + "}";
		...
		if (this.unterschrift) {
			closing += TEXUMBRUCH + "\\\\includegraphics[width=3.5cm]{"
					+ optionen.getProperty("verzeichnistexdateien")
					+ "Sign.jpg}";
			String rep = RechnungData.TEXUMBRUCH
		...
		if (std_ersetzung) {
			latexcode = latexcode.replaceAll("%%tabelle%%", TEXUMBRUCH
					+ stundentabelle + TEXUMBRUCH + "\n");
			if (this.stunden)
				...
				latexcode = latexcode.replaceAll("%%summe%%",
						"insgesamt " + zf.format(this.summe) + TEXEURO);
			latexcode = latexcode.replaceAll("%%zahlfrist%%",
					df.format(this.zahlungsdatum.getTime()));
		} else {
			...
			latexcode = latexcode.replaceAll(ersetzung, zf.format(this.summe)
					+ " " + TEXEURO);
			ersetzung = "\"Monat\"";
			...
			String new_monat = einheit_dat.getDisplayName(Calendar.MONTH, Calendar.LONG, Locale.GERMANY)
					+ " "
					+ einheit_dat.get(java.util.GregorianCalendar.YEAR);
		...
		try {
			String command = "pdflatex -halt-on-error -output-directory "
					+ optionen.getProperty("arbeitsverzeichnis") + " "
					+ outputfile_tex;
	...
	private String createTable() {
		...
		if (std_ersetzung) {
			...
			if (this.stunden)
				tabelle += " & Std.";
			if (this.zusatz1_name != null)
				tabelle += " & " + this.zusatz1_name;
			if (this.zusatz2_name != null)
				tabelle += " & " + this.zusatz2_name;
			...
			for (int i = 0; i < einheiten.size(); i++) {
				...
				if (this.stunden)
					tabelle += einheiten.elementAt(i).getDauer() / 60 + " & "; // Minuten
																				// in
																				// Stunden
				if (zusatz1_name != null)
					tabelle += einheiten.elementAt(i).getZusatz1() + " & ";
				if (zusatz2_name != null)
					tabelle += einheiten.elementAt(i).getZusatz2() + " & ";
				tabelle += zf.format(einheiten.elementAt(i).getPreis()) + " "
			...
			for (int i = 0; i < einheiten.size(); i++) {
				...
				zeile = zeile.replaceAll("\"Datum\"",
						df.format(einheiten.elementAt(i).getDatum()));
				...
				if (iminute < 10)
					...
					minute = "" + iminute;
				zeiten = kalender.get(java.util.GregorianCalendar.HOUR_OF_DAY)
						+ ":" + minute;
				zeiten += " - ";
				...
				zeile = zeile.replace("\\hline", TEXUMBRUCH + TEXUMBRUCH
						+ TEXUMBRUCH + "hline");
				zeile += "\n";
				tabelle += zeile;
			}
		...
		if (pdfProg != null) {
			if (this.latexergebnis == 0 && this.pdf != null) {
				...
				if (pdf_datei.canRead()) {
					...
					System.err.println("Pdfdatei existiert nicht: "
							+ pdf_datei.getAbsolutePath());
				}
			}
		} else {
	...
	public int speichern() {
		...
		java.sql.Date sql_rechnungsdatum = new java.sql.Date(
				this.rechnungsdatum.getTimeInMillis());
		java.sql.Date sql_zahlungsdatum = new java.sql.Date(
				this.zahlungsdatum.getTimeInMillis());
		...
		String befehl = "mv " + this.pdf + " "
				+ optionen.getProperty("verzPdfFiles") + new_pdf;
		...
		if (ergebnis == 0) {
			befehl = "mv " + this.tex_out + " "
					+ optionen.getProperty("verzPdfFiles") + dateiname + ".tex";
			...
			try {
				...
				try {
					...
					ergebnis = proc.exitValue();
					if (proc.exitValue() == 0) {
						this.tex_out = optionen.getProperty("verzPdfFiles")
								+ dateiname + ".tex";
					}
				} catch (InterruptedException e2) {
					logger.debug("waitfor pdflatex");
		...
		if (ergebnis == 0) {
			...
			if (this.neu) { // Bei neuer Rechnung INSERT
				...
				sql = "UPDATE rechnungen SET" + " datum=\""
						+ sql_rechnungsdatum + "\", rechnungnr=\""
						+ this.rechnungsnr + "\", betrag=" + this.summe
				...
				if (this.zusatz2_name == null)
					...
					sql += "\", zusatz2=\"1";
				if (this.zusammenfassungen_erlauben)
					sql += "\", zusammenfassungen=\"1";
				else
					sql += "\", zusammenfassungen=\"0";
			...
			if (verbindung.sql(sql)) {
				...
				// Elemente, die bei dublikatsuche aussortiert wurden, werden in
				// Liste aufgenommen.
